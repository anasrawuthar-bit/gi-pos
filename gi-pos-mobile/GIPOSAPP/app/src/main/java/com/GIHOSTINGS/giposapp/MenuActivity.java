...
public class MenuActivity extends InsetActivity {
  ...
  private void build(){
    ...
    Button back=button("‹",Color.WHITE,ink);back.setTextSize(22);back.setOnClickListener(v->finish());head.addView(back,new LinearLayout.LayoutParams(dp(44),dp(44)));
    ...
    search=new EditText(this);search.setSingleLine();search.setHint("Search item name or alias");search.setHintTextColor(muted);search.setTextColor(ink);search.setTextSize(16);search.setPadding(dp(15),0,dp(15),0);search.setBackground(shape(Color.WHITE,11,1,line));LinearLayout.LayoutParams searchParams=new LinearLayout.LayoutParams(-1,dp(50));searchParams.setMargins(dp(16),dp(4),dp(16),0);root.addView(search,searchParams);search.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int st,int c,int a){}public void onTextChanged(CharSequence s,int st,int b,int c){loadItems();}public void afterTextChanged(Editable e){}});
  ...
  private void addCategory(){EditText name=new EditText(this);name.setHint("Category name");name.setHintTextColor(muted);name.setTextColor(ink);name.setSingleLine();name.setPadding(dp(12),0,dp(12),0);new AlertDialog.Builder(this).setTitle("New category").setView(name).setNegativeButton("Cancel",null).setPositiveButton("Add",(d,w)->{String value=name.getText().toString().trim();if(!value.isEmpty()){categoryId=database.addCategory(value);refreshCategories();loadItems();}}).show();}
