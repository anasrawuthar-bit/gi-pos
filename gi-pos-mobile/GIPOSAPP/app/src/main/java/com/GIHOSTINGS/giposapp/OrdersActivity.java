...
public class OrdersActivity extends InsetActivity {
  ...
  private void build(){
    ...
    LinearLayout header=row();header.setGravity(Gravity.CENTER_VERTICAL);header.addView(AppBackButton.create(this,v->finish()),new LinearLayout.LayoutParams(dp(44),dp(44)));LinearLayout title=column();title.setPadding(dp(10),0,0,0);title.addView(text("Orders",24,ink,true));title.addView(text("Resume service or review today's bills",13,muted,false));header.addView(title,new LinearLayout.LayoutParams(0,-2,1));root.addView(header);
