...
public final class PrintDispatcher {
  ...
  private static void process(Context context){PosDatabase db=PosDatabase.get(context);for(PosDatabase.PrintJobInfo job:db.pendingPrintJobs()){PosDatabase.PrinterProfile printer=db.printerFor(job.type);if(printer==null){String error="Configure a "+("kot".equals(job.type)?"KOT":"bill")+" printer";db.updatePrintJob(job.id,"failed",error);showFailure(context,job,error);continue;}try{db.updatePrintJob(job.id,"printing","");byte[] bytes=EscPosFormatter.format(job.type,job.payload,printer.paperWidth);if(bytes.length<10)throw new IllegalStateException("Print document is empty");send(context,printer,bytes);db.updatePrintJob(job.id,"done","");}catch(Exception error){boolean failed=job.attempts+1>=3;String detail=message(error);db.updatePrintJob(job.id,failed?"failed":"retry",detail);if(failed)showFailure(context,job,detail);}}}
  ...
  private static void showFailure(Context context,PosDatabase.PrintJobInfo job,String error){String document="kot".equals(job.type)?"KOT":"Bill";String detail=document+" print failed: "+error;new Handler(Looper.getMainLooper()).post(()->Toast.makeText(context,detail,Toast.LENGTH_LONG).show());}
