...
public class TableActivity extends InsetActivity {
  ...
  private void build(){
    ...
    Button back=button("‹",Color.WHITE,ink);back.setOnClickListener(v->finish());head.addView(back,new LinearLayout.LayoutParams(dp(44),dp(44)));
  ...
  private void addFloor(){EditText name=input("Floor or area name",InputType.TYPE_CLASS_TEXT);new AlertDialog.Builder(this).setTitle("Add floor / area").setView(name).setNegativeButton("Cancel",null).setPositiveButton("Add",(d,w)->{if(!name.getText().toString().trim().isEmpty()){selectedFloor=database.addFloor(name.getText().toString());refresh();}}).show();}
  private void addTable(){List<PosDatabase.FloorInfo> floors=database.floors();if(floors.isEmpty()){addFloor();return;}String floor=selectedFloor==null?floors.get(0).id:selectedFloor;LinearLayout form=dialogForm();EditText name=input("Table name",InputType.TYPE_CLASS_TEXT);EditText seats=input("Seats",InputType.TYPE_CLASS_NUMBER);seats.setText("4");form.addView(name);form.addView(seats,top(10));new AlertDialog.Builder(this).setTitle("Add table").setView(form).setNegativeButton("Cancel",null).setPositiveButton("Add",(d,w)->{String value=name.getText().toString().trim();if(!value.isEmpty())database.addTable(floor,value,number(seats,4));refresh();}).show();}
  private void editTable(PosDatabase.TableInfo table){LinearLayout form=dialogForm();EditText name=input("Table name",InputType.TYPE_CLASS_TEXT);name.setText(table.name);EditText seats=input("Seats",InputType.TYPE_CLASS_NUMBER);seats.setText(String.valueOf(table.seats));form.addView(name);form.addView(seats,top(10));new AlertDialog.Builder(this).setTitle("Edit "+table.name).setView(form).setNegativeButton("Cancel",null).setPositiveButton("Save",(d,w)->{if(!name.getText().toString().trim().isEmpty())database.updateTable(table.id,name.getText().toString(),number(seats,table.seats));refresh();}).show();}
  private LinearLayout dialogForm(){LinearLayout form=column();form.setPadding(dp(22),dp(8),dp(22),0);return form;}
  private EditText input(String hint,int type){EditText v=new EditText(this);v.setHint(hint);v.setHintTextColor(muted);v.setTextColor(ink);v.setInputType(type);v.setSingleLine();v.setPadding(dp(12),0,dp(12),0);v.setBackground(shape(Color.rgb(248,250,252),9,1,Color.rgb(199,211,223)));v.setLayoutParams(new LinearLayout.LayoutParams(-1,dp(50)));return v;}
